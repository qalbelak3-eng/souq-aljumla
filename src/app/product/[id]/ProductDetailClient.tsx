...
import { PublicProduct, Product, SaleType } from '@/types';
...
export default function ProductDetailClient({ initialProduct, productId }: ProductDetailClientProps) {
  ...
  const [product, setProduct] = useState<PublicProduct | null>(initialProduct || null);
  ...
  useEffect(() => {
    // If we already have initialProduct, only fetch related products
    if (initialProduct) {
      ...
      if (initialProduct.category) {
        fetch(`/api/products?category=${encodeURIComponent(initialProduct.category)}`)
          .then((r) => r.json())
          .then((relData) => {
            if (relData.success) {
              setRelatedProducts(
                relData.products.filter((p: Product) => p.id !== initialProduct.id).slice(0, 4)
              );
            }
          })
    ...
    // Client fallback fetching if not pre-rendered
    setIsLoading(true);
  ...
  const handleAddToCart = () => {
    // PublicProduct is safe to add to cart (compatible with Product as omitted fields are optional)
    addToCart(product as Product, quantity, saleType);
    setAdded(true);
