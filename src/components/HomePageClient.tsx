  ...
  return (
    <div className="space-y-5 sm:space-y-6 pb-32 overflow-x-hidden w-full max-w-full">
      ...
      {topShowcases.map((showcase) => (
        <section key={showcase.id} className="w-full max-w-[1240px] mx-auto px-0 sm:px-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-3">
            {categories
              ...
              .map((cat) => (
                ...
                >
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center mb-1.5 transition duration-300 group-hover:scale-105">
                    <CategoryIcon name={cat.name} icon={cat.icon} image={cat.image} animate={false} />
                  </div>
